...
for ii = 1:length(classification.names)
    tractname = strrep(strrep(strrep(strrep(strrep(classification.names{ii},'.','_'),' ','_'),'+','_'),'-','_'),'.','_');
...
for ifg = 1:length(fg_classified)
    fg_filename = strrep(strrep(strrep(strrep(strrep(fg_classified{ifg}.name,'.','_'), ' ', '_'), '+','_'),'-','_'),'.','_');
    try
